...
def check_num(file_name, threshold=15000):
    data_file = h5py.File(file_name, 'r')
    xyz = np.array(data_file['xyz_v'], dtype='float32')
    data_file.close()
    # print('xyz {}'.format(xyz.shape))
    if xyz.shape[0] < threshold:
        return False
    else:
        return True
...
def read_structure(file_name, read_geof):
    ...
    xyz = np.array(data_file['xyz_v'], dtype='float32')
    rgb = np.array(data_file['rgb_v'], dtype='float32')
    elevation = np.array(data_file['elevation'], dtype='float32')
    xyn = np.array(data_file['xyn_v'], dtype='float32')
    edg_source = np.array(data_file['source'], dtype='int').squeeze()
    edg_target = np.array(data_file['target'], dtype='int').squeeze()
    is_transition = np.array(data_file['is_transition'], dtype='uint8')
    objects = np.array(data_file['objects'][()])
    labels = np.array(data_file['labels_v'], dtype='uint32').squeeze().astype(dtype='int32')
...
def create_semanticposs_datasets(args, logger, test_seed_offset=0):
    ...
    for fname in sorted(os.listdir(path), key=lambda x:os.stat(path + "/" + x).st_size):
        ...
        if fname.endswith(".h5"):
            if '{}_'.format(args['test_area']) in fname: continue
            # break
            path_file = os.path.join(path, fname)
            # logger.info('path_file: {}'.format(path_file))
            # exit()
            #flag = check_num(path_file, args['num_point'])
            # print('flag: {}'.format(flag))
            # exit()
            # if 'scene0106_01' in path_file: continue
            #if flag:
            #    trainlist.append(path_file)
            #else:
            #    cnt += 1
            trainlist.append(path_file)
    
    #logger.info('less that 14999 cnt: {}'.format(cnt))
    logger.info('train list: {}'.format(len(trainlist)))
    #exit()
    # ----------------- test -----------------
    # path = args['data_root_test']     # for true test
    path = args['data_root']           # for true train
    for fname in sorted(os.listdir(path),key=lambda x:os.stat(path + "/" + x).st_size):
        if fname.endswith(".h5"):
            if '{}_'.format(args['test_area']) not in fname: continue
            path_file = os.path.join(path, fname)
            testlist.append(path_file)
...
def graph_loader(entry, train, args, logger, db_path, test_seed_offset=0, full_cpu = False):
    ...
    #logger.info('short_name: {}'.format(short_name))
    raw_xyz = np.array(xyz)
    raw_rgb = np.array(rgb)
    raw_labels = np.array(labels)

    one_hot = np.eye(args['classes'])


    # rgb = rgb/255
    #logger.info('raw_labels shape: {} type: {} {}'.format(raw_labels.shape, raw_labels.dtype, raw_labels[:3]))
    #logger.info('raw_labels max: {}'.format(np.max(raw_labels)))
    #logger.info('raw_labels min: {}'.format(np.min(raw_labels)))
    
    raw_labels = one_hot[raw_labels]
    # logger.info('raw_labels shape: {} type: {} {}'.format(raw_labels.shape, raw_labels.dtype, raw_labels[:3, :]))
...
def my_collate_semanticposs(batch):
