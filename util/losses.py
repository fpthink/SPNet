...
def compute_loss(args, is_transition, weights_loss):
...
def compute_weight_loss(args, objects, edg_source, edg_target, is_transition, pred_components, pred_in_component):
...
def SEAL_loss(num_point, pred_components, pred_in_component, objects, edg_source, edg_target, is_transition, pfea, transition_factor=5):
    ...
    # print('pfea:', pfea.shape)
    diff = compute_dist(pfea, edg_source, edg_target, 'euclidian')
    # print('diff:', diff.shape)

    # print(np.unique(pred_in_component).shape)
    # print(np.unique(pred_in_component)[:10])

    aa, ss, pp = np.unique(pred_in_component, return_index=True, return_inverse=True)
    tmp = np.zeros((num_point,), dtype='uint32')
    for i in range(num_point):
        tmp[pred_in_component[i]] = pp[i]
    ...
    #         ,   w_per_component[pred_in_component[edg_target[is_transition.nonzero()]]])).max(0)  * transition_factor# 1 if not transition 1+w otherwise
    SEAL_weights[is_transition.nonzero()] += np.stack(\
                (w_per_component[tmp[pred_in_component[edg_source[is_transition.nonzero()]]]]
            ,   w_per_component[tmp[pred_in_component[edg_target[is_transition.nonzero()]]]])).max(0)  * transition_factor# 1 if not transition 1+w otherwise
    
    weights_loss = torch.from_numpy(SEAL_weights).cuda()
    ...
    inter_edg = is_transition==1
    loss2 = (zhang(torch.sqrt(diff[inter_edg]+1e-10), weights_loss[inter_edg], 'euclidian')).sum()
    # print(loss1, loss2)
    # print(loss1.item(), loss2.item())
    # exit()
    #return loss1/ weights_loss.sum(), loss2/ weights_loss.sum()
    return loss1/len(edg_source), loss2/len(edg_source)
