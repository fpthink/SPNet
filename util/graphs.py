...
def compute_graph_nn_2(xyz, k_nn1, k_nn2, voronoi = 0.0):
    ...
    distances = distances[:, 1:]
    #---knn2---
    target2 = (neighbors.flatten()).astype('int64')
    #---knn1-----
    if voronoi>0:
        tri = Delaunay(xyz)
        graph["source"] = np.hstack((tri.vertices[:,0],tri.vertices[:,0], \
              tri.vertices[:,0], tri.vertices[:,1], tri.vertices[:,1], tri.vertices[:,2])).astype('int64')
        graph["target"]= np.hstack((tri.vertices[:,1],tri.vertices[:,2], \
              tri.vertices[:,3], tri.vertices[:,2], tri.vertices[:,3], tri.vertices[:,3])).astype('int64')
        ...
        graph["target"] =  np.hstack((graph["target"],np.transpose(neighbors.flatten(order='C')).astype('int32')))
        ...
        graph["source"] = np.matlib.repmat(range(0, n_ver)
            , k_nn1, 1).flatten(order='F').astype('int64')
        graph["target"] = np.transpose(neighbors.flatten(order='C')).astype('int64')
        graph["distances"] = distances.flatten().astype('float32')
    #save the graph
    return graph, target2

def compute_graph_nn_2_v1(xyz, k_nn1, k_nn2, voronoi = 0.0):
    """compute simulteneoulsy 2 knn structures
    only saves target for knn2
    assumption : knn1 <= knn2"""
    assert k_nn1 <= k_nn2, "knn1 must be smaller than knn2"
    ...
    #---knn2---
    # target2 = (neighbors.flatten()).astype('int64')
    target2 = neighbors.astype('int64')
    #---knn1-----
    if voronoi>0:
        tri = Delaunay(xyz)
        graph["source"] = np.hstack((tri.vertices[:,0],tri.vertices[:,0], \
...
def compute_sp_graph(xyz, d_max, in_component, components, labels, n_labels):
    ...
    for i_com in range(0, n_com):
        ...
        if has_labels and label_hist:
            # graph["sp_labels"][i_com, :] = sum(labels[comp,:]) # le
            graph["sp_labels"][i_com, :] = np.sum(labels[comp,:])
...
def compute_sp_graph_yj(xyz, d_max, in_component, components, labels, n_labels, logger=None):
    """compute the superpoint graph with superpoints and superedges features"""
    n_com = max(in_component)+1
    # logger.info('n_com: {}'.format(n_com))
    in_component = np.array(in_component)
    # # logger.info('unique in_component: {}'.format(np.unique(in_component)))
    # # n_com = len(np.unique(in_component))
    # # logger.info('n_com: {}'.format(n_com))
    # u, in_component = np.unique(in_component, return_inverse=True)
    # # logger.info('in_component: {}'.format(in_component))
    # n_com = max(in_component)+1
    # # logger.info('n_com: {}'.format(n_com))
    # # exit()
    ...
    graph["se_point_count_ratio"] = np.zeros((n_sedg, 1), dtype='float32')
    if has_labels:
        # graph["sp_labels"] = np.zeros((n_com, n_labels + 1), dtype='uint32')  # le
        graph["sp_labels"] = np.zeros((n_com, n_labels), dtype='uint32')
    else:
        graph["sp_labels"] = []
    #---compute the superpoint features---
    for i_com in range(0, n_com):
        # logger.info('i_com: {}'.format(i_com))
        # logger.info('components: {}'.format(len(components[i_com])))
        comp = components[i_com]
